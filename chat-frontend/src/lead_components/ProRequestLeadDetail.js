...
function ProRequestLeadDetail() {
  ...
  useEffect(() => {
    async function fetchData(){
      try {
        const rq_pro_lead = await axios.get(`${base}/apis/markets/pro_request/${id}/ `);
        setLead(rq_pro_lead.data);
        setLoaded(true);
      }
      catch(err){
  ...
  const handleClaim = (event) => {
    ...
    try {
        const claimLead = axios({
          method: "PATCH",
          url: `${base}/apis/markets/pro_request/${id}/cl_or_re/ `,
          data: {
            claimer:[tokenUserId],
          },
          config: {
            ...
            { menuItem: 'Property Info', render: () =>(
              ...
              {lead.qualified === true ? (
                ...
                <CustomItemGroup relaxed='very'>
                      ...
                      <Item>
                        <Item.Content>
                          <Item.Header style={{'fontSize':'14px'}}>Pro Requested</Item.Header>
                          <Item.Meta>
                            <span className='rqplPropertyType' style={{'fontSize':'14px'}}>{lead.type_of_proffesional}</span>
                          </Item.Meta>
                        </Item.Content>
                      </Item>
                </CustomItemGroup>
                ...
                <Item.Group relaxed='very' style={{'backgroundColor':'#d0f3e969','padding':'10px'}}>
                      <Item>
                        <Item.Content>
                          ...
                          <Item.Meta>
                            <span className='rqplPropertyAdetails' style={{'fontSize':'13px'}}>{lead.service_details}</span>
                          </Item.Meta>
                        </Item.Content>
                    </Item>
                </Item.Group>
...
export default ProRequestLeadDetail;
