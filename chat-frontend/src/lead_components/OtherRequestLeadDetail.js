...
function OtherRequestLeadDetail() {
  ...
  useEffect(() => {
    async function fetchData(){
      try {
          const rq_pro_lead = await axios.get(`${base}/apis/markets/other_service_request/${id}/ `);
  ...
  const handleClaim = (event) => {
    ...
    try {
        const claimLead = axios({
          method: "PATCH",
          url: `${base}/apis/markets/other_service_request/${id}/cl_or_re/ `,
          data: {
            claimer:[tokenUserId],
          },
          config: {
...
export default OtherRequestLeadDetail;
