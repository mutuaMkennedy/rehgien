...
import PropertyRequestStats2 from './PropertyRequestStats2.js';
import PropRequestModalContent from './PropertyRequestLeadModalContent.js';
...
class PropertyRequests extends Component {
  constructor(props){
    super(props);
    this.state = {
    lead:[],
  ...
  async componentDidMount(){
    ...
    const rq_property_leads = await axios.get(`${base}/apis/markets/request_property/ `)
    await this.setState({
      lead:rq_property_leads.data,
      loading:false,
    })
  }
  ...
  handleRefer(event){
    ...
    const referLead = axios({
      method: "PATCH",
      url: `${base}/apis/markets/request_property/${pk}/cl_or_re/ `,
      data: {
          ...
          return(
            <StyledCard centered key={item.pk}>
                <Card.Content>
                      ...
                      <Card.Description>
                        {
                          item.additional_details.slice(0,20) + '...'
                        }
                      </Card.Description>
                </Card.Content>
                <Card.Content extra textAlign='left' style={{'display':'flex', 'justifyContent': 'flex-start','alignItems':'top','flexGrow':'1' }}>
                    <SectionDivider/>
                    <div style={{'width':'100%'}}>
                        ...
                        <Item.Group style={{'display':'flex', 'justifyContent': 'flex-start', 'alignItems':'top'}}>
                            <Item style={{'margin':'0', 'marginLeft':'5px'}}>
                                  <Item.Content style={{'fontSize':'10px'}} verticalAlign='top'>
                                  <Item.Header>
        ...
        <FilterHideButton id='hideFilter' onClick={hideFilters}>
          ...
          {this.state.loading === true ? (
            <EmptyResults>
              ...
              <PropertyRequestStats leads={leads}/>
            )
          }
          </LeftSidebar>
          <RightSidebar id='rightSideBar'>
            ...
            {this.state.loading === true ? (
              ...
              <>
                <PropertyRequestStats2 leads={leads}/>
