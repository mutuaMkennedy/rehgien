...
export default class ProRequestModalContent extends Component {
  constructor(props){
    super(props);
    this.state = {
      show:false,
      ...
      { menuItem: 'Request Info', render: () =>(
        ...
        {this.props.details.qualified === true ? (
          ...
          'alignItems':'top','backgroundColor':'#d0f3e969', 'padding':'10px'}}>
                ...
                <Item>
                  <Item.Content>
                    <Item.Header style={{'fontSize':'14px'}}>Pro Type</Item.Header>
                    <Item.Meta>
                      <span className='rqplPropertyType' style={{'fontSize':'14px'}}>{this.props.details.type_of_proffesional}</span>
                    </Item.Meta>
                  </Item.Content>
                </Item>
          </Item.Group>
