...
export default class AgentLeadRequestStats extends PureComponent {
  state = {
    activeIndex:0,
    _activeIndex:0,
  };
  ...
  render() {
    ...
    var sellersArray =  this.props.leads.filter(function(request) {
        return request.ownership === "BUY";
    });
    var rentersArray =  this.props.leads.filter(function(request) {
        return request.ownership === "RENT";
    });
    var lessorsArray =  this.props.leads.filter(function(request) {
    ...
    const data = [
      { name: 'Sellers', value: sellersArray.length, color:'#0088FE'},
      { name: 'Renters', value: rentersArray.length, color:'#00C49F'},
      { name: 'Lessors', value: lessorsArray.length, color:'#FFBB28'},
    ];
